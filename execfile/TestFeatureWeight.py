...
# 对某个文件进行向量转换
def changefiletolist(filename):
    file = codecs.open(filename, "r", encoding = "utf8")
    filecontent = file.read()
    filewordslist = jieba.cut(filecontent)

    filewords = []
    for eachword in filewordslist:
        filewords.append(eachword)
    return filewords

# 对某个文件进行TFIDF计算并存到.svm中

def calfiletfidf(filename, feature, dffeaturedic):
    resultfile = open(filename.strip(".txt")+ ".svm", "w")
    resultfile.close()
    resultfile = open(filename.strip(".txt")+ ".svm", "a")
    resultfile.write(str(5) + " ")
    filewords = changefiletolist(filename)
    for i in range(len(feature)):
        if feature[i] in filewords:
            currentfeature = feature[i]
            featurecount = filewords.count(feature[i])
            tf = float(featurecount)/(len(filewords))
            # 计算逆文档频率
            idffeature = math.log(float(TrainDocumentCount+1)/(int(dffeaturedic[currentfeature])+2))
            featurevalue = idffeature * tf
            # resultfile.write(str(i+1)+":"+str(featurevalue) + " ")
            resultfile.write(str(i+1)+":"+str(1) + " ")
    resultfile.write("\n")
    resultfile.close()
